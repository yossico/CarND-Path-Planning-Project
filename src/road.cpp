...
//Holds  all the Lanes which holds all the vehicle in the road and updates them
...
bool Road::safe_lane(Vehicle& car, LANE lane) {
	//iterate on all cars in the ane and check for minimum distance
...
LANE Road::lane_change_available(Vehicle& car) {
	...
	else { // if car is in center lane check both right & left lane
